...
class SignalementAnalyzer
{
    ...
    public function analyze(string $description): array
    {
        ...
        if ($apiKey && class_exists(AI::class)) {
            try {
                $response = AI::chat()->create([
                    ...
                    'messages' => [
                        ...
                        [
                            'role' => 'user',
                            'content' => $this->buildAnalysisPrompt($description),
                        ],
                    ],
                    'temperature' => 0.2,
                ]);
                ...
                Log::warning('AI SignalementAnalyzer chat call failed: ' . $e->getMessage());
            }
        }

        if (!$aiResult && $apiKey && filter_var($endpoint, FILTER_VALIDATE_URL)) {
            try {
                ...
                Log::warning('AI SignalementAnalyzer HTTP call failed: ' . $e->getMessage());
            }
        }
    ...
    {
        return "Analyse le signalement suivant: \"{$description}\".\n" .
